...
## 三、部署前端
前端需要你下载到本地，在本地执行前端文件的构建，再上传到服务器中使用。  
因为需要修改前端文件中的管理员邮箱，当在后面服务器不能正常访问的时候，显示到页面中提示用户。
...
```
会在当前目录中新增一个名为 diary 的文件夹，里面就是日记项目的前端源码。
...
yarn
```
你会看到有进度条在走，等它走完会有文件夹中多出一个名为 `node_modules` 的文件夹
...
```
会在 web 目录中创建一个为名 **portal** 的目录，里面放置好了后台程序的所有文件。 **portal** 的目录结构是这样的
...
3. 完成之后，服务就正常运行在了你机器的 3000 端口，当你用浏览器访问 `你的ip:3000` 时，比如我的 `kylebing.cn:3000` 应该会看到下面的信息：
    ```json
    {"status":"success","message":"Portal API is running","title":"Portal for Diary"}
    ```
...
接下来要做的就是将 `localhost:3000` 的服务代理到 `portal` 上去，这个就需要修改 `nginx` 配置文件。

如何找到 nginx 配置文件目录和配置文件自行豆包，不同系统对于网站的配置文件可能会有不同，但配置文件的内容是一致的。
...
需要修改 `/config/project_config.json` 文件内容，改成你的七牛云配置。

```json
  "qiniu_img_base_url": "", // 空间域名，最后面带 `/`
  "qiniu_bucket_name": "", // 七牛云对象存储空间的名称
  "qiniu_style_suffix": "", // 七牛云图片样式后缀，缩放图片尺寸
```

### 3. 和风天气配置 `[选配]`
> 如若不配置：只是不能自动获取当地天气和温度而已  
用于从 [和风天气](https://www.qweather.com/) 中获取地域的天气和温度信息，也是在 `/config/project_config.json` 文件中
...
```json
  ...
  "hefeng_weather_api_host": "",   // 和风天气改用了 api host 的模式
```
